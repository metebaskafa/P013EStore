...
namespace P013EStore.WebAPIUsing.Controllers
{
    public class AccountController : Controller
    {
        ...
        private readonly string _apiAdres = "https://localhost:44335/api/AppUsers";
        ...
        public async Task<ActionResult> Index()
        {
            ...
            if (userId is null)
            {
                ...
                return RedirectToAction("Login"); // login olmamışsa logine gitsin indexe giremesin
        ...
        public async Task<IActionResult> SignIn(AppUser appUser)
        {
            try
            {
                var kullanicilar = await _httpClient.GetFromJsonAsync<List<AppUser>>(_apiAdres);
                var kullanici = kullanicilar.FirstOrDefault(x => x.Email == appUser.Email);
                if (kullanici != null)
                {
